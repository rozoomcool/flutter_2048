  ...
  Widget build(BuildContext context) {
    return BlocBuilder<GameBloc, GameState>(
      builder: (BuildContext context, state) => GestureDetector(
        onVerticalDragEnd: (details) => context.read<GameBloc>().add(GameDragEvent(0, details.velocity.pixelsPerSecond.dy)),
        onHorizontalDragEnd: (details) => context.read<GameBloc>().add(GameDragEvent(details.velocity.pixelsPerSecond.dx, 0)),
        child: Container(
          ...
          child: GridView.builder(
            ...
            itemBuilder: (BuildContext context, int index) {
              ...
              child: state.cells[row][col] != 0 ? Cell(value: state.cells[row][col]) : const CellEmpty());
            },
          ),
        ),
      ),
